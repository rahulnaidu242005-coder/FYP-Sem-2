...
def inject_runtime_token(token: str) -> Callable:
    ...
    def call_api(since: str, until: str) -> str:
        ...
        since_norm = '2024-02-01T00:00:00Z' # normalize_ts(since)
        until_norm = '2026-12-31T23:59:59Z' # normalize_ts(until)

        incidents = NETWITNESS_CLIENT_INSTANCE.get_incidents(
            token=token, since=since_norm, until=until_norm
        )
        items = incidents.get("items", [])
        with open('inchidents.json', 'w') as file:
            # Perform file operations within this indented block
            file.write(json.dumps(items[0:5]))
        # for inc in items:
        #     slim.append({
        #         "id": inc.get("id"),
        #         "name": inc.get("name"),
...
model = ChatOllama(
    ...
    # model="gpt-oss:20b",
    # temperature=0.2,
    # Remove the comma and comment the line 165 out -> uses Jason's compute
    # base_url="http://bazzite.tail3be278.ts.net:11434"
...
#         - You only have access to the tool called `call_api` to fetch incident data from NetWitness.
#         - Only call the call_api tool when the user asks for incident data between two timestamps, and if the chat context does not already contain that data.
# # 
#         CONTEXT USAGE
...
#         """.strip()
    template = """
        # ROLE
        You are "Netty", an expert SOC (Security Operations Center) Assistant. Your primary function is to interpret telemetry data and provide actionable risk assessments.

        # CORE PROTOCOL: THINK-THEN-ACT
        You must follow a strict Chain of Thought (CoT) process for every turn.
        1. **Thought**: Analyze if the user is asking for data/status. Check if that data exists in the current conversation context.
        2. **Action**: If data is missing or a search is required, call the `call_api` tool and answer
        3. **Response**: If data is present, provide the assessment using the Netty persona.
