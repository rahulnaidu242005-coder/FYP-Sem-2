...
from lib.Config import ANALYST_LEVEL, USERNAME, PASSWORD, get_prompt
...
def get_token(username: str = "your_username", password: str = "your_password") -> str:
    is_healthy = NETWITNESS_CLIENT_INSTANCE.health_check()
    token_str = "dummy_token"
    if not is_healthy:
        logger.error("NetWitness API is unreachable. Cannot get token.")
        raise Exception("NetWitness API health check failed.")
    """Gets authentication token from NetWitness API."""
    try:
        token_str = NETWITNESS_CLIENT_INSTANCE.get_token(username=username, password=password)  # Replace dummies
    except:
        logger.error("Failed to retrieve token from NetWitness API.")
        raise
    return token_str
